...
function WP_members_list_search_fields_actions() {
	...
	if(isset($_REQUEST['action']) or isset($_REQUEST['action2'])) {
		$action = (isset($_REQUEST['action2']) and !empty($_REQUEST['action2'])) ? $_REQUEST['action2'] : $_REQUEST['action'];
	}
	else {
		return;
	}
	...
	switch($action) {
		...
		case 'field' :
			...
			if(isset($o['searches']) and is_array($o['searches']) and in_array($_POST['field'],(array)$o['searches'])) {
				$getWP->addError('This field has already been added.');
				return;
			}
			$o['searches'] = is_array($o['searches']) ? $o['searches'] : array();
		...
		case 'delete' :
			...
			if(!isset($_REQUEST['searches']) or empty($_REQUEST['searches'])) {
...
function WP_members_list_search_fields() {
	...
	$o = $getWP->getOption('tern_wp_members',$tern_wp_members_defaults);
	include(MEMBERS_LIST_DIR.'/view/search.php');
	include(MEMBERS_LIST_DIR.'/view/search-form.php');
}
